    ...
    public void write(MovieCollection movieCollection) throws FileNotFoundException, FilePermissionException, CustomIOException {
        ...
        try (BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(path)))) {
            ...
            for (Movie movie : movieHashMap.values()) {
                ...
                bufferedWriter.write(indent.repeat(2) + "<actor>\n");
                bufferedWriter.write(indent.repeat(3) + "<actorName>" + movie.getactor().getName() + "</actorName>\n");
                bufferedWriter.write(indent.repeat(3) + "<birthday>" + movie.getactor().getBirthday() + "</birthday>\n");
                bufferedWriter.write(indent.repeat(3) + "<weight>" + movie.getactor().getWeight() + "</weight>\n");
                if (movie.getactor().getsalary() == null) {
                    bufferedWriter.write(indent.repeat(3) + "<salary/>\n");
                } else {
                    bufferedWriter.write(indent.repeat(3) + "<salary>" + movie.getactor().getsalary() + "</salary>\n");
                }
                bufferedWriter.write(indent.repeat(2) + "</actor>\n");
                bufferedWriter.write(indent.repeat(1) + "</movie>\n");
            }
